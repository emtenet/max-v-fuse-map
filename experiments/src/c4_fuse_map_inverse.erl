...
inverse_m(Density, XX, YY, MM, Acc) ->
    case c4_fuse_map:to_name({{c4, XX, YY}, {mux, MM}, direct_link}, Density) of
        {ok, {{c4, X, Y}, {interconnect, I}, direct_link}} ->
            X = XX,
            inverse(Density, YY, MM, X, Y, I, Acc);
...
inverse(Density, YY, MM, X, Y, I, Acc) ->
    case Acc of
        ...
        #{Density := #{I := #{Y := #{X := {YYY, MMM}}}}} ->
            throw({
                Density, {{c4, X, YY}, {mux, MM}},
                interconnect, I,
                existing, {{c4, X, YYY}, {mux, MMM}}
            });

        #{Density := Is = #{I := Ys = #{Y := Xs}}} ->
            Acc#{Density => Is#{I => Ys#{Y => Xs#{X => {YY, MM}}}}};

        #{Density := Is = #{I := Ys}} ->
            Acc#{Density => Is#{I => Ys#{Y => #{X => {YY, MM}}}}};

        #{Density := Is} ->
            Acc#{Density => Is#{I => #{Y => #{X => {YY, MM}}}}};

        _ ->
            Acc#{Density => #{I => #{Y => #{X => {YY, MM}}}}}
    end.
