-module(global_interconnect_mux_playground).

-export([run/0]).

% This experiment looks for pairs of fuses that could form the
% Globalinterconnect mux.
%
% Only found for 5M570Z and higher.
%
% 5M240Z shares the {iob, 1, 3} interconnect with the global clocks.
...
block(Density, {local_interconnect, X, Y}, Indexes, Acc0) ->
    case density:is_global(X, Y, Density) of
        true ->
            Global = {global, X, Y},
            io:format(" ==> ~s ~w~n", [Density, Global]),
            route_cache:fold_indexes(
                fun (Index, Froms, Acc) ->
                    interconnect(Density, Global, Index, Froms, Acc)
                end,
                Acc0,
                Indexes
            );
...
interconnect(Density, Global, Index, Froms, Acc0) ->
    %io:format(" ==> ~p ~p~n", [Global, Index]),
    case experiments(Froms) of
        [] ->
            Acc0;

        Experiments0 ->
            froms(Density, Global, Index, Experiments0, Acc0)
    end.

%%--------------------------------------------------------------------

froms(Density, _Global, Index, Experiments0, Acc0) ->
...
from(N, Ns, [_ | Froms], Matrix, Index, Acc0) ->
    ...
    case matrix:pattern_match(Matrix, Pattern) of
        [{_, Fuse}] ->
            Acc = interconnects_add_one(Index, Fuse, Acc0),
...
remove_fuse(Density, Location) ->
    case fuse_map:to_name(Location, Density) of
        {ok, {{global, _}, {interconnect, _}, _}} ->
            false;

        {ok, {{global, _}, {interconnect, _}, _, _}} ->
            false;
...
interconnects_add_one(Index, Fuse, Acc) ->
    Key = fuse_key(Fuse),
    %io:format("   ~-18w <- ~s ~w~n", [{interconnect, Index}, Side, Key]),
    interconnects_add(Index, Key, direct_link, Acc).
